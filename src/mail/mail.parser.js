...
class MailParser{
  ...
  prepareJson(result){
    ...
    packageDetails['AccountManagerPhone '] = this.getValue(result,'Phone','MercuryGate','MasterBillOfLading',0,'ReferenceNumbers',0,'ReferenceNumber');
    ...
    packageDetails['BrokerName'] = 'Alpha Transportation';
    ...
    packageDetails['DispatcherInfo'] = this.getValue(result,'Phone','MercuryGate','MasterBillOfLading',0,'Carriers',0,'Carrier',0,'Address',0,'Contacts',0,'Contact',0,'ContactMethods',0,'StatusDetails');
  ...
  getPickupDropArray(result){
    ...
    for(var i=0;i<plan.length;i++){
      ...
      if(plan[i]['key'] && plan[i]['key']['type'] && plan[i]['key']['type'] === 'Pickup' ){
        ...
        obj[`${key} Status`] = this.getValue(plan[i],null,'Status',0)
        ...
        obj[`${key} Appointment Time`] = this.getValue(plan[i],'planned','Dates',0,'Date')
